...
ALPHAS      <- c(0, 0.5, 1)
ALPHA_NAMES <- c("0" = "Ridge (α=0)", "0.5" = "Elastic Net (α=0.5)", "1" = "LASSO (α=1)")
...
auc_df$alpha_label <- ALPHA_NAMES[auc_df$alpha]

vlines <- bind_rows(lapply(names(cv_list), function(a) {
  data.frame(alpha = a, alpha_label = ALPHA_NAMES[a],
             lambda_1se = log(cv_list[[a]]$lambda.1se),
             best_auc   = max(cv_list[[a]]$cvm))
}))

p1 <- ggplot(auc_df, aes(log_lambda, mean_auc, color = alpha)) +
  geom_ribbon(aes(ymin = lo, ymax = hi, fill = alpha), alpha = 0.15, color = NA) +
  geom_line(linewidth = 0.8) +
  geom_vline(data = vlines, aes(xintercept = lambda_1se, color = alpha),
             linetype = "dotted") +
...
sparsity_df <- bind_rows(lapply(names(cv_list), function(a) {
  ...
  for (lam in c("lambda.min", "lambda.1se")) {
    ...
    nsel  <- sum(coefs != 0)
    auc_v <- cv$cvm[which.min(abs(cv$lambda - cv[[lam]]))]
    data.frame(alpha = as.numeric(a), lambda_rule = lam,
               n_selected = nsel, best_auc = auc_v)
  }
}))

p2 <- ggplot(sparsity_df, aes(alpha, n_selected, color = lambda_rule,
                               group = lambda_rule)) +
